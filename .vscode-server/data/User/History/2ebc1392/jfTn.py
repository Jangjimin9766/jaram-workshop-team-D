...
# Reinitialize pin setup after cleanup
gpio.setmode(gpio.BCM)
gpio.setup(TRIGER, gpio.OUT)
gpio.setup(ECHO, gpio.IN)
gpio.setup(LED, gpio.OUT)
...
# Add edge detection
gpio.add_event_detect(SWITCH, gpio.RISING, callback=switch_pressed, bouncetime=300)
...
try:
    while True:
        ...
        if MIN_DIST <= dist1 <= MAX_DIST:
            ...
            # Once the LED is on, we don't turn it off again unless switch is pressed
            if not led_on:
                gpio.output(LED, gpio.LOW)
